...
模型在**动作/运镜上呼吸**，在**事实上不许漂**：
```
FACT-LOCK
- 身份/服装/标志物：<subject> 或 @图片N，一字不差
- 场景/环境 reference：Clip 级必须声明场景图 @图片N（即便不逐分镜嵌，也不能丢——防场景漂移）
- 道具：允许出现的道具白名单；禁新增道具
- 场景人口：空镜/仅主角/主角+背景群像（沿用 Phase4 场景人口字段）
- 不可发明：禁加新角色/新地点/新剧情事件/新台词
- 关键 beat：必须命中的 1~3 个剧情锚点（起点状态 / 转折 / 情绪落点）
- 放权范围：仅「关键 beat 之间的衔接动作、运镜演化、动量」可由模型推断
```

### 5.5 Phase8 专用格式（不伪装成普通 8-A）
infer-between Clip 用独立块，便于解析器/校验器区分严控 8-A 与质感 infer-between：
```
X-TECH INFER-BETWEEN CLIP
FACT-LOCK:
  [§5.4 七项，含场景 @图片N + population state]
REFERENCE AUTHORITY CONTRACT:
  IDENTITY / STRUCTURE / STAGING / LOOK / ACTING 各指向谁（见 §4）
TRIPTYCH ROLE:
  Top panel    = start state
  Center panel = progression pressure
  Bottom panel = payoff / final frame
  （声明 triptych 控 look 还是只控结构——OAK 原生控 look；混身份板时按 Contract 收窄）
...
```
- infer-between Clip **豁免** Phase8「每个中间动作/分镜都逐拍 + 逐分镜 `@图片N`」硬清单；**但 Clip 级场景 `@图片N` 与 population 必须保留**。校验器对此类 Clip 走豁免分支（见 §9 validator 待办）。

### 5.6 松生成 + 严审片
- **松生成**：按上面格式给即梦发挥空间。
- **严审片**：用 video-sop 现成 **Gate10 逐帧审片**（按目标根探测：Claude 根 `phase9-frame-audit.md`；+ `Desktop\tools\video-frame-audit\frame_audit.py`）拿 FACT-LOCK 逐帧核身份/道具/朝向/关键 beat/场景人口；漂了按 Gate10 返修（重抽/局部删/改 Prompt）。
- 一句话：**放权放在动作和运镜上，不放在事实上；松着生成、严着审片。**

### 5.7 ⚠️ validator 现状（未扩展前的操作约束）
现有 `validate_phase8_prompt.py` 只认 8-A 结构（分镜时段/镜头五段/场景嵌入…），**不认 `X-TECH INFER-BETWEEN CLIP` 格式**。在 §9 待办的 validator 扩展落地前：
- 含 infer-between Clip 的 Prompt 集，**不能拿 8-A 校验器的 `RESULT: PASS` 当该 Clip 的交付门**（它会 FAIL/报错）。
- 操作：严控 8-A 的 Clip 照常跑校验器；**infer-between Clip 走人工闸7 复核 + Gate10 严审片**把关，不依赖确定性校验器。
- 故 infer-between 在 validator 扩展前属**实验路线**：先单 Clip 小范围试、过 Gate10 验证后再推广。
...
**待办（不在本批，需双根对齐后再动）**：
- `validate_phase8_prompt.py`：① C1 不混写检查；② infer-between Clip 豁免「逐分镜 `@图片N`」硬断言、改校 FACT-LOCK 含场景 ref + population；③ FACS `AU\d+` 正则 / Laban·IPA 语义 gate。**validator 最后动、先双根对齐**（Claude 根 + Codex `GPT长视频创作\` 根，走 HANDOFF + COLLABORATION.md）。
- **Gate10 审片 reference 双根命名分歧**：Claude 根 `phase9-frame-audit.md` / Codex 根 `post-generation-video-audit.md`——实施前探测目标根实际文件名，长期建议统一命名或别名。
