...
- **待实测后补 infer-between 金样示例**：本模块现只给 schema/格式模板，**不放未实测填实例**（防未验证内容当金样被照抄）；待一条 infer-between Clip 真过目标平台 + Gate10，再把验证过的 prompt 补进 `§11 示例`（性质同 `phase8-video-prompt.md` §五官方原文示例）。
...
- `003_OAK_triptych_to_目标平台.md`（Route B 三联画）
